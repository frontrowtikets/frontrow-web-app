...
const qrcode = useQRCode(`${appURL}/verify/movie/ticket/${props.ticketId}/${props.transactionDetails.id}/${props.userDetails.id}`);

const screeningDate = computed(() => {
    const thedate = moment(props.theatre.screening_date).format(
        "ddd, DD MMM YYYY"
    );
    return thedate;
});
const startTime = computed(() =>{
    const starts = moment(props.theatre.start_time, "HH:mm:ss").format("h:mm A")
    return starts
}

);
const endTime = computed(() =>{
    const ends = moment(props.theatre.end_time, "HH:mm:ss").format("h:mm A");
    return ends
}
...
<template>
    <div class="container py-4" id="movieticketx">
            ...
            <div class="purple-border">
                <div class="shadow ticket-main">
                    ...
                    >
                        ...
                        <p class="mb-0">
                            {{ props.seatDetails?.seatmap?.room_name }}
                        </p>
                    </div>

                    <div class="p-4 ticket-body">
                        <div class="mb-4 movie-info">
                        ...
                        <div class="mb-4 seats-section">
                            <p class="mb-2 label">Booked Seat</p>
                            <div class="p-3 seats-container bg-light">
                                <span class="badge bg-secondary">{{
                                    props.seatDetails.seat_number
                                }}</span>
                            </div>
                        </div>
