                ...
                <div class="mb-4 col-12 d-flex justify-content-between align-items-center">
                    ...
                    <div class="">
                        <b-dropdown variant="light" right>
                            ...
                            <b-dropdown-item>
                                <b-form-checkbox id="checkbox-1" name="checkbox-1" :value="true"
                                    :unchecked-value="false" checked>
                                    All
                                </b-form-checkbox>
                                <b-form-checkbox v-for="(cat, index) in props.categories" :key="`${index}_${cat.id}`"
                                    id="checkbox-1" name="checkbox-1" :value="true" :unchecked-value="false">
                                    {{ cat.name }}
                                </b-form-checkbox>
                            </b-dropdown-item>
                        </b-dropdown>
                ...
                <b-container>
                    <div class="flex-wrap gap-4 pt-4 d-flex align-items-center justify-content-center">
                        <div v-for="(item, index) in paginatedItems" :key="index" class="col-xl-3 col-md-4 col-sm-6">
                            <EventCard :eventName="item.title" :eventImageLink="item.thumbnail_url"
                                :eventDate="item.start_date" :eventLocation="item.location_name" :eventId="item.id"
                                :eventTickets="item.event_tickets" @view="viewEvent" />
                        </div>
                    </div>
