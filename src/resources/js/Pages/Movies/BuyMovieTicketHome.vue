...
<template>
    <section>
        ...
        <b-container class="" style="margin-top: 16em">
            <HomeHeader />
            <PageHeader :title="props.buyMovieDetails.title" :items="state.items" role="button"
                    ...
                    <div class="col-12">
                        <div class="card">
                            <div class="card-body d-flex align-items-center justify-content-center">
                                <MovieCheckout :paymentDetails="cleanedSelectedSeats"
                                    :currency="selectedTheatre?.currency" :total="totalPrice"
                                    :movieId="props.buyMovieDetails.id" :myWallet="props.myWallet" />
                            </div>
                        </div>
                    </div>
                </div>
                            ...
                            <div class="card-body">
                                <div class="w-100 d-flex justify-content-between">
                                    <b-button variant="light" disabled>
                                        <div style="font-weight: bold;">Select seats to Proceed</div>
                                    </b-button>
                                ...
                                <div class="mt-4">
                                    <div v-if="props.buyMovieDetails.seatmap.length > 0">
                                        <div v-for="(theSeatMap, theSeatMapIndex) in seatMapFields"
                                            :key="`${theSeatMapIndex}_${theSeatMap.room}_${theSeatMapIndex}`">
                                            <TheSeatMap :rowData="theSeatMap.seats" :reserved="theSeatMap.reserved"
                                                :theatre="theSeatMap.theatre" :roomName="theSeatMap?.room"
                                                :roomId="theSeatMap?.roomId" :showMarkReservedButton="false"
                                ...
                                <div class="mt-5 text-start">
                                    <div v-if="props.buyMovieDetails.seatmap.length > 0">
                                        <div v-if="cleanedSelectedSeats.length > 0">
                                            ...
                                            <div v-for="(item, index) in cleanedSelectedSeats"
                                                :key="`${item.roomId}_${index}`">
                                                <div class="mb-2"><span class="fw-bold me-3">Theatre:</span>{{
                                                    item?.theatre.theatre }}</div>
                                                <div class="mb-2"><span class="fw-bold me-3">Room:</span>{{
                                                    item?.roomName }}</div>
                                                <div class="mb-2"><span class="fw-bold me-3">Tickets:</span>{{
                                                    item?.selectedSeats.length }}</div>
                                                <div class="mb-4">
                                                    <span class="fw-bold me-3">Seats:</span><span
                                                        class="me-3 badge badge-soft-secondary"
                                                        v-for="(seat, index) in item?.selectedSeats" :key="index">{{
                                                            seat }}</span>
                                                </div>
                                            </div>
                                        </div>
                                    ...
                                    <div class="mb-4 text-center">
                                        ...
                                        <div>
                                            <h4>{{ selectedTheatre?.currency || "UGX" }} {{
                                                useCurrencyFormat(totalPrice) }}</h4>
                                        </div>
                                    </div>
                                    <div class="mt-5 d-flex justify-content-center mt-sm-0"
                                        @click="totalPrice > 0 ? (showCheckout = true) : null">
                                        <a class="btn btn-success flex-fill"> <i
                                                class="mdi mdi-cart-arrow-right me-1"></i> Proceed </a>
                                    </div>
                                </div>
                            </div>
                        </div>
                        ...
                        <div class="mb-5 card-body">
                            <div class="pb-4 w-100 d-flex justify-content-between align-items-center">
                                ...
                                <div class="" @click="movieDetails">
                                    <a class="btn btn-light"> <i class="mdi mdi-eye-outline" v-if="width > 424"></i>
                                        View Details </a>
                                </div>
                            </div>
                            <div>
                                <div v-if="props.buyMovieDetails.show_times.length == 0"
                                                ...
                                                <div class="p-3 card-body bg-hover" role="button">
                                                    <div class="d-flex align-items-center justify-content-between">
                                                        <div class="avatar-xs me-3" @click="viewMovieDetails">
                                                            <div class="bg-transparent rounded avatar-title">
                                                                <i
                                                                    class="bx bx-play-circle font-size-24 text-secondary"></i>
                                                            </div>
                                                        </div>
                                                        <div style="color: black; font-weight: bold" class="text-muted">
                                                            {{
                                                                moment(theShowtime.showTime[0].screening_date).format(`ddd,
                                                            DD MMM YYYY`) }} At
                                                    ...
                                                    <div class="mt-1">
                                                        ...
                                                        <span class="ms-3">
                                                            ...
                                                            {{ useCurrencyFormat(theShowtime.showTime[0].ticket_price)
                                                            }}</span>
                                                    </div>
                                                </div>
                                            </div>
