...
import MovieTicketList from "@/js/Components/MovieTicketList.vue";
...
<template>
    <DashboardLayout>
        ...
        <Head title="Manage Movie - Ticket Sales" />
                        ...
                        <b-tab title="Ticket Sales - Grid" active>
                            <div v-if="paginatedItems.length > 0">
                                <div class="flex-wrap gap-5 mt-5 d-flex justify-content-center">
                                    <MovieTicketCard v-for="(ticket, index) in paginatedItems"
                                        :key="`${index}_${ticket.id}`" :ticketId="ticket.ticket_id"
                                        :status="ticket.ticket_status" :movie="ticket?.movie" :theatre="ticket?.theatre"
                        ...
                        <b-tab title="Ticket Sales - List">
                            <div v-if="paginatedItems.length > 0">
                                <table class="table table-striped table-bordered dt-responsive nowrap w-100">
                                    ...
                                    <tbody>
                                        <MovieTicketList v-for="(ticket, index) in paginatedItems"
                                            :key="`${index}_${ticket.id}`" :ticketId="ticket.ticket_id"
                                            :status="ticket.ticket_status" :movie="ticket?.movie"
                                            :theatre="ticket?.theatre" :userDetails="ticket?.user_payment_detail"
                                            :transactionDetails="ticket?.payment_transaction"
                                            :seatDetails="ticket?.show_time_seats" />
                                    </tbody>
                                </table>
